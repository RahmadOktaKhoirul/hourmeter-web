...
export default function Settings({ darkMode, onToggleDark }: SettingsProps) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        <h2 className="text-xl font-bold text-on-surface">Pengaturan</h2>
        <p className="text-sm text-on-surface-variant mt-0.5">Konfigurasi sistem dan preferensi</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
          ...
          {tabs.map((tab) => (
            <button key={tab} onClick={() => setActiveTab(tab)}
              className={cn('w-full text-left px-4 py-2.5 rounded-lg text-sm font-medium transition-all',
                activeTab === tab ? 'bg-primary text-on-primary' : 'text-on-surface-variant hover:bg-surface-container-high hover:text-on-surface'
              )}>
              {tab}
            </button>
          ))}
        ...
        <div className="lg:col-span-9 space-y-5">
          ...
          {activeTab === 'General' && (
            <>
              <section className="bg-surface-container-low p-6 rounded-xl border border-outline-variant/10 space-y-5">
                <h3 className="font-semibold text-base text-on-surface border-b border-outline-variant/10 pb-4">Konfigurasi Umum</h3>
                <div className="space-y-5">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                    <div className="space-y-1.5">
                      <label className="text-xs font-medium text-on-surface-variant ml-1">Bahasa Sistem</label>
                      <div className="relative">
                        <Icons.Languages className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-on-surface-variant" />
                        <select value={language} onChange={e => setLanguage(e.target.value)}
                          className="w-full bg-surface-container-high border border-outline-variant/20 rounded-lg py-2.5 pl-10 pr-4 text-sm text-on-surface focus:ring-2 focus:ring-primary/30 outline-none appearance-none">
                    ...
                    <div className="space-y-1.5">
                      <label className="text-xs font-medium text-on-surface-variant ml-1">Zona Waktu</label>
                      <div className="relative">
                  ...
                  <div className="space-y-4 pt-4">
                    ...
                    <div className="flex items-center justify-between p-4 rounded-lg bg-surface-container-high/30">
                      <div>
                ...
                <div className="pt-6 border-t border-outline-variant/10 flex justify-end gap-4">
                  <button onClick={() => { setLanguage('English (US)'); setTimezone('UTC +07:00 (Jakarta)'); }}
                    className="px-4 py-2 text-sm font-medium text-on-surface-variant hover:text-on-surface transition-colors">Batalkan</button>
                  <button onClick={handleSave} disabled={saving}
                    className={cn('px-5 py-2 text-sm font-medium rounded-lg hover:opacity-90 transition-opacity disabled:opacity-60',
              ...
              <section className="bg-error/5 p-5 rounded-xl border border-error/10 space-y-4">
                <h3 className="font-semibold text-base text-error">Zona Berbahaya</h3>
                <p className="text-sm text-on-surface-variant">Tindakan ini tidak dapat dibatalkan.</p>
                <div className="flex flex-wrap gap-3">
                  <button onClick={handlePurgeLogs} className="px-4 py-2 border border-error/20 text-error text-sm font-medium rounded-lg hover:bg-error/10 transition-colors">
                    Hapus Log Telemetri
                  </button>
                  <button onClick={handleDeactivateUnit} className="px-4 py-2 bg-error text-on-error text-sm font-medium rounded-lg hover:opacity-90 transition-opacity">
                    Nonaktifkan Semua Unit
                  </button>
                </div>
              </section>
            </>
          ...
          {activeTab === 'Security' && (
            <section className="bg-surface-container-low p-6 rounded-xl border border-outline-variant/10 space-y-5">
              <h3 className="font-semibold text-base text-on-surface border-b border-outline-variant/10 pb-4">Security Settings</h3>
              <div className="space-y-6">
                <div className="flex items-center justify-between p-4 rounded-lg bg-surface-container-high/30">
                  <div>
                  ...
                  <span className="text-xs font-medium px-2 py-0.5 rounded-md bg-error/10 text-error">Nonaktif</span>
                </div>
                <div className="flex items-center justify-between p-4 rounded-lg bg-surface-container-high/30">
                  <div>
          ...
          {activeTab === 'Notifications' && (
            <section className="bg-surface-container-low p-6 rounded-xl border border-outline-variant/10 space-y-5">
              <h3 className="font-semibold text-base text-on-surface border-b border-outline-variant/10 pb-4">Notification Preferences</h3>
              <div className="space-y-4">
                {[
                  ...
                  <div key={item.label} className="flex items-center justify-between p-4 rounded-lg bg-surface-container-high/30">
                    <div>
          ...
          {activeTab === 'Integrations' && (
            <section className="bg-surface-container-low p-6 rounded-xl border border-outline-variant/10 space-y-5">
              <h3 className="font-semibold text-base text-on-surface border-b border-outline-variant/10 pb-4">Integrations</h3>
              <div className="space-y-4">
                {[
                  ...
                  <div key={item.name} className="flex items-center justify-between p-4 rounded-lg bg-surface-container-high/30 border border-outline-variant/10">
                    ...
                    )}>{item.connected ? 'Terhubung' : 'Tidak Terhubung'}</span>
                  </div>
                ))}
              </div>
            </section>
          ...
          {activeTab === 'Billing' && (
            <section className="bg-surface-container-low p-6 rounded-xl border border-outline-variant/10 space-y-5">
              <h3 className="font-semibold text-base text-on-surface border-b border-outline-variant/10 pb-4">Billing & Plan</h3>
              <div className="p-5 rounded-xl bg-primary/10 border border-primary/20">
                <p className="text-xs text-primary mb-1">Paket Saat Ini</p>
                <p className="text-2xl font-bold text-on-surface">Enterprise</p>
