...
const chartData = [
  { name: 'MON', hours: 8.2 },
  { name: 'TUE', hours: 10.1 },
  { name: 'WED', hours: 12.4, active: true },
  { name: 'THU', hours: 8.8 },
  { name: 'FRI', hours: 10.5 },
  { name: 'SAT', hours: 5.2 },
  { name: 'SUN', hours: 2.1 },
...
export default function Machines() {
  ...
  async function loadMachines() {
    const { data } = await supabase.from('machines').select('*, business_units(id, name, location)').order('created_at');
    if (data) {
      setMachines(data);
  ...
  useEffect(() => {
    loadMachines();
    supabase.from('business_units').select('*').order('name').then(({ data }) => { if (data) setBusinessUnits(data); });
  }, []);
  ...
  async function handleScheduleService(e: React.FormEvent) {
    ...
    if (!selected) return;
    setSaving(true);
    const nextHm = serviceForm.next_service_hm ? parseFloat(serviceForm.next_service_hm) : selected.current_hm + (selected.service_interval ?? 500);
    await Promise.all([
      ...
      supabase.from('machines').update({ service_status: 'SCHEDULED', hours_to_service: nextHm - selected.current_hm }).eq('id', selected.id),
      supabase.from('telemetry_logs').insert({ machine_id: selected.id, event_type: 'SERVICE', title: `Service Scheduled by ${serviceForm.performed_by}`, description: serviceForm.notes || null }),
    ]);
    setSaving(false);
  ...
  async function handleAddShift(e: React.FormEvent) {
    ...
    await Promise.all([
      ...
      supabase.from('telemetry_logs').insert({ machine_id: selected.id, event_type: 'OPERATOR_SWAP', title: `Shift logged: ${shiftForm.operator_name}`, description: `HM ${shiftForm.hm_start} → ${shiftForm.hm_end || 'ongoing'}` }),
    ]);
    setSaving(false);
    setShowShift(false);
  ...
  async function handleEditMachine(e: React.FormEvent) {
    ...
    if (!selected) return;
    setSaving(true);
    await supabase.from('machines').update(editForm).eq('id', selected.id);
    setSaving(false);
    setShowEdit(false);
    loadMachines();
  }
  ...
  function generateReport() {
    ...
    const a = document.createElement('a'); a.href = url; a.download = `report-${selected.machine_code}.csv`; a.click();
    URL.revokeObjectURL(url);
  }

  if (loading) return <div className="flex items-center justify-center h-64 text-on-surface-variant text-sm">Loading machines...</div>;
  if (!selected) return <div className="flex items-center justify-center h-64 text-on-surface-variant text-sm">No machines found.</div>;
        ...
        {machines.map((m) => (
          <button key={m.id} onClick={() => setSelected(m)}
            className={cn('px-4 py-2 rounded-xl text-xs font-bold uppercase tracking-widest whitespace-nowrap transition-all',
              selected.id === m.id ? 'bg-primary text-on-primary shadow-lg shadow-primary/20' : 'bg-surface-container-high text-on-surface-variant hover:text-on-surface'
            )}>
            {m.machine_code}
          </button>
        ))}
      ...
      <section className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-end">
        <div className="lg:col-span-8 flex flex-col gap-2">
          <div className="flex items-center gap-4">
            <span className={cn('w-3 h-3 rounded-full', selected.status === 'RUNNING' ? 'bg-primary animate-pulse shadow-[0_0_12px_rgba(84,224,131,0.6)]' : 'bg-on-surface-variant')} />
            <span className={cn('font-headline font-bold text-sm tracking-[0.3em] uppercase', selected.status === 'RUNNING' ? 'text-primary' : 'text-on-surface-variant')}>
              {selected.status}
            </span>
          </div>
        ...
        <div className="lg:col-span-4 flex justify-end gap-3 flex-wrap">
          <button onClick={() => { setEditForm({ machine_code: selected.machine_code, unit_name: selected.unit_name, serial_number: selected.serial_number ?? '', status: selected.status, service_status: selected.service_status, current_hm: selected.current_hm, previous_hm: selected.previous_hm, hours_to_service: selected.hours_to_service ?? 500, business_unit_id: selected.business_unit_id ?? '' }); setShowEdit(true); }}
          ...
          <button onClick={generateReport} className="px-5 py-3 bg-surface-container-high text-on-surface font-bold rounded-2xl hover:bg-surface-container-highest transition-all uppercase tracking-widest text-xs flex items-center gap-2">
            <Icons.Download className="w-4 h-4" /> Report
          </button>
          <button onClick={() => { setServiceForm({ performed_by: '', notes: '', next_service_hm: '' }); setShowService(true); }}
              ...
              {metric.progress !== undefined ? (
                <div className="space-y-2">
                  ...
                  <div className="w-full bg-surface-container-highest h-1.5 rounded-full overflow-hidden">
                    <div className={cn('h-full rounded-full', metric.progress >= 90 ? 'bg-error' : 'bg-tertiary')} style={{ width: `${metric.progress}%` }} />
                  </div>
                </div>
              ) : (
      ...
      <section className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        <div className="lg:col-span-8 bg-surface-container-low rounded-3xl p-10 flex flex-col gap-8">
          <div>
            ...
            <p className="text-[10px] font-bold text-on-surface-variant uppercase tracking-[0.2em] mt-1">Last 7 days</p>
          </div>
          <div className="h-80 w-full">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={chartData} margin={{ top: 20, right: 0, left: -20, bottom: 0 }}>
                ...
                <Tooltip cursor={{ fill: 'rgba(84,224,131,0.05)' }} contentStyle={{ backgroundColor: 'var(--c-surface-container-high)', border: 'none', borderRadius: '12px', color: 'var(--c-on-surface)' }} labelStyle={{ fontWeight: 700 }} />
                <Bar dataKey="hours" radius={[8, 8, 0, 0]} barSize={60}>
                  {chartData.map((entry, index) => <Cell key={`cell-${index}`} fill={entry.active ? 'var(--c-primary)' : 'var(--c-surface-container-highest)'} />)}
                </Bar>
              </BarChart>
            </ResponsiveContainer>
          </div>
        ...
        <div className="lg:col-span-4 bg-surface-container-low rounded-3xl p-10 flex flex-col gap-6">
          <div className="flex justify-between items-center">
            ...
            <button onClick={() => { setShiftForm({ operator_name: '', hm_start: String(selected.current_hm), hm_end: '' }); setShowShift(true); }}
          ...
          <div className="flex flex-col gap-2 flex-1">
            {shifts.length === 0
              ...
              : shifts.map((shift, i) => (
                  <div key={shift.id} className={cn('grid grid-cols-2 py-4 px-6 rounded-2xl transition-all', i === 0 ? 'bg-primary/10 border border-primary/20' : 'bg-surface-container-highest/30')}>
          ...
          {logs.length === 0
            ...
            : logs.map((log) => (
                <div key={log.id} className="px-8 py-5 flex items-center gap-6 hover:bg-surface-container-high/30 transition-colors">
                  <span className={cn('text-[10px] font-bold px-2 py-1 rounded-lg uppercase tracking-widest shrink-0',
      ...
      <Modal open={showService} onClose={() => setShowService(false)} title={`Schedule Service — ${selected.machine_code}`}>
        <form onSubmit={handleScheduleService} className="space-y-4">
          ...
          <Field label="Performed By">
            <input required className={inputCls} placeholder="Technician name" value={serviceForm.performed_by} onChange={e => setServiceForm(f => ({ ...f, performed_by: e.target.value }))} />
          </Field>
          <Field label="Next Service at HM (h)">
            <input type="number" step="0.1" className={inputCls} placeholder={String(selected.current_hm + (selected.service_interval ?? 500))} value={serviceForm.next_service_hm} onChange={e => setServiceForm(f => ({ ...f, next_service_hm: e.target.value }))} />
          </Field>
          <Field label="Notes">
            <textarea className={inputCls + ' resize-none'} rows={3} placeholder="Service notes..." value={serviceForm.notes} onChange={e => setServiceForm(f => ({ ...f, notes: e.target.value }))} />
          </Field>
          <div className="flex justify-end gap-3 pt-2">
            <button type="button" onClick={() => setShowService(false)} className="px-5 py-2.5 text-xs font-bold uppercase tracking-widest text-on-surface-variant hover:text-on-surface transition-colors">Cancel</button>
      ...
      <Modal open={showShift} onClose={() => setShowShift(false)} title={`Log Shift — ${selected.machine_code}`}>
        <form onSubmit={handleAddShift} className="space-y-4">
          <Field label="Operator Name">
            <input required className={inputCls} placeholder="Operator name" value={shiftForm.operator_name} onChange={e => setShiftForm(f => ({ ...f, operator_name: e.target.value }))} />
          </Field>
          <div className="grid grid-cols-2 gap-4">
            <Field label="HM Start (h)">
              <input required type="number" step="0.1" className={inputCls} value={shiftForm.hm_start} onChange={e => setShiftForm(f => ({ ...f, hm_start: e.target.value }))} />
            </Field>
            <Field label="HM End (h) — optional">
              <input type="number" step="0.1" className={inputCls} placeholder="Leave blank if active" value={shiftForm.hm_end} onChange={e => setShiftForm(f => ({ ...f, hm_end: e.target.value }))} />
            </Field>
          </div>
          <div className="flex justify-end gap-3 pt-2">
            <button type="button" onClick={() => setShowShift(false)} className="px-5 py-2.5 text-xs font-bold uppercase tracking-widest text-on-surface-variant hover:text-on-surface transition-colors">Cancel</button>
            <button type="submit" disabled={saving} className="px-6 py-2.5 bg-primary text-on-primary font-bold rounded-xl text-xs uppercase tracking-widest hover:opacity-90 disabled:opacity-60">
        ...
        <form onSubmit={handleEditMachine} className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            <Field label="Machine Code">
              <input required className={inputCls} value={editForm.machine_code ?? ''} onChange={e => setEditForm(f => ({ ...f, machine_code: e.target.value }))} />
            </Field>
            <Field label="Unit Name">
              <input required className={inputCls} value={editForm.unit_name ?? ''} onChange={e => setEditForm(f => ({ ...f, unit_name: e.target.value }))} />
            </Field>
          </div>
          <Field label="Business Unit">
            <select className={selectCls} value={editForm.business_unit_id ?? ''} onChange={e => setEditForm(f => ({ ...f, business_unit_id: e.target.value || null }))}>
          ...
          <div className="grid grid-cols-2 gap-4">
            <Field label="Status">
              <select className={selectCls} value={editForm.status ?? 'STOPPED'} onChange={e => setEditForm(f => ({ ...f, status: e.target.value as Machine['status'] }))}>
                <option>RUNNING</option><option>STOPPED</option><option>MAINTENANCE</option>
              </select>
            </Field>
            <Field label="Service Status">
              <select className={selectCls} value={editForm.service_status ?? 'OK'} onChange={e => setEditForm(f => ({ ...f, service_status: e.target.value as Machine['service_status'] }))}>
          ...
          <div className="grid grid-cols-2 gap-4">
            <Field label="Current HM (h)">
              <input type="number" step="0.1" className={inputCls} value={editForm.current_hm ?? 0} onChange={e => setEditForm(f => ({ ...f, current_hm: parseFloat(e.target.value) }))} />
            </Field>
            <Field label="Hours to Service (h)">
              <input type="number" step="0.1" className={inputCls} value={editForm.hours_to_service ?? 500} onChange={e => setEditForm(f => ({ ...f, hours_to_service: parseFloat(e.target.value) }))} />
            </Field>
          </div>
          <div className="flex justify-end gap-3 pt-2">
            <button type="button" onClick={() => setShowEdit(false)} className="px-5 py-2.5 text-xs font-bold uppercase tracking-widest text-on-surface-variant hover:text-on-surface transition-colors">Cancel</button>
            <button type="submit" disabled={saving} className="px-6 py-2.5 bg-primary text-on-primary font-bold rounded-xl text-xs uppercase tracking-widest hover:opacity-90 disabled:opacity-60">
